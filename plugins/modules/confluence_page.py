...
from atlassian.errors import ApiConflictError, ApiError, ApiNotFoundError, ApiPermissionError, ApiValueError
...
def create_confluence_instance(url, username, password):
    ...
    if ".atlassian.net" in url:
        confluence_module = Confluence(
            url=url, username=username, password=password, cloud=True)
    else:
        confluence_module = Confluence(
            url=url, username=username, password=password, cloud=False)


def _page_exists(module, space_key, title):
    is_page_existing = False
    try:
        is_page_existing = confluence_module.page_exists(
            space=space_key, title=title)
    except ApiNotFoundError as e:
        module.fail_json(msg='Page with {title} not found', exception='{e}')
...
def _get_page_id(module, space, title):
    page_id = None
...
def _handle_present(module, space_key, title, body, overwrite, labels, parent_page_title=None, update_title_to=None):

    # Seed module return
    result = dict(changed=False)
    space_has_page = _page_exists(module, space_key, title)
    confluence_response = None
    has_labels = (labels is not None) and len(labels) > 0
    parent_id = None

    if parent_page_title:
        parent_id = _get_page_id(module, space_key, parent_page_title)

    # If page with title already exists return the page instead of try to create it
    if space_has_page and not overwrite:
        try:
            confluence_response = confluence_module.get_page_by_title(
                space_key, title)
        except ApiNotFoundError as e:
            module.fail_json(msg='Page with {title} not found', exception='{e}')
        except ApiPermissionError:
    ...
    if not space_has_page:
        try:
            confluence_response = confluence_module.create_page(
                space=space_key, title=title, body=body, type="page", representation="wiki", parent_id=parent_id)
        except ApiNotFoundError as e:
            module.fail_json(msg='Page with {title} not found', exception='{e}')
        except ApiPermissionError:
            module.fail_json(msg='Please check if your user have the appropiate permissions', exception='{e}')
    ...
    if has_labels and result['changed']:
    ...
    if confluence_response is not None:
        ...
        module.fail_json(
            changed=False, msg='Something went wrong creating or updating the page')
...
def _handle_absent(module, space_key, title, recursive=False):
    ...
    if not space_has_page:
        module.exit_json(
            changed=False, msg='Page does not exist, no action was taken.')

    page_id = _get_page_id(module, space_key, title)

    try:
        confluence_module.remove_page(
            page_id, status=None, recursive=recursive)
    except ApiNotFoundError as e:
        module.fail_json(msg='Page with {title} not found', exception='{e}')
    except ApiPermissionError:
        module.fail_json(msg='Please check if your user have the appropiate permissions', exception='{e}')
...
def _handle_move(module, space_key, target_title, title):
    ...
    space_has_page = _page_exists(module, space_key, title)
    space_has_target_page = _page_exists(module, space_key, target_title)
    ...
    page_to_move = _get_page_id(module, space_key, title)
    page_to_target = _get_page_id(module, space_key, target_title)

    try:
        confluence_response = confluence_module.move_page(
            space_key, page_to_move, page_to_target, position="append")
        result['changed'] = True
...
def _handle_append_page(module, space_key, title, body, parent_page_title):
    ...
    if space_has_page:
        try:
            confluence_response = confluence_module.append_page(
                page_id, title=title, append_body=body, parent_id=parent_id, type="page", representation="wiki", minor_edit=False)
            result['changed'] = True
...
def _handle_prepend_page(module, space_key, title, body, parent_page_title):
    ...
    if space_has_page:
        try:
            confluence_response = confluence_module.prepend_page(
                page_id, title=title, prepend_body=body, parent_id=parent_id, type="page", representation="wiki", minor_edit=False)
            result['changed'] = True
...
def _handle_get_page(module, space, title):
    ...
    space_has_page = _page_exists(module, space, title)
...
def _handle_get_page_cql(module, cql, start=0, limit=None, expand=None, include_archived_spaces=None, excerpt=None):
    ...
    try:
        confluence_response = confluence_module.cql(
            cql, start=0, limit=None, expand=None, include_archived_spaces=None, excerpt=None)
        result['response'] = confluence_response
    except ApiNotFoundError as e:
        module.fail_json(msg='Page with {title} not found', exception='{e}')
    except ApiPermissionError:
...
def run_module():
    module_args = dict(
        ...
        state=dict(type='str', choices=('present', 'absent', 'move', 'add_labels',
                   'remove_labels', 'append_page', 'prepend_page', 'page', 'cql')),
        space_key=dict(type='str'),
        title=dict(type='str'),
        update_title_to=dict(type='str', default=None),
        body=dict(type='str'),
    ...
    title = params['title']
    update_title_to = params['update_title_to']
    body = params['body']
    ...
    if state in ['present']:
        _handle_present(module, space_key, title, body,
                        overwrite, labels, parent_page_title, update_title_to)
        ...
        _handle_get_page_cql(module, cql, start=0, limit=None,
